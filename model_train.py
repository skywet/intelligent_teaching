...
def construct_model(people):
    ...
    '''
    model = Sequential()
    model.add(Convolution2D(64,(3,3),strides=1,padding='same',activation='relu',input_shape=(56,56,3),data_format='channels_last'))
    ...
    model.add(MaxPooling2D(pool_size=3,strides=1,padding='same'))
    model.add(Convolution2D(32, 3, strides=1, padding='same', activation='relu',data_format='channels_last'))
    model.add(MaxPooling2D(pool_size=3,strides=1,padding='same'))
